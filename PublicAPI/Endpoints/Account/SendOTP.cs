        ...
        ForgetPasswordResponse>
    {
            ...
            CancellationToken cancellationToken = new())
        {
            try
            {
                ...
                if (existedUser is null) throw new ApplicationException("Email not exist");
                var random = new Random();
                var otp = random.Next(100000, 999999).ToString();
                await _emailSender.SendEmail(request.Email, MessageBuilder.BuildSubject(request.Lang, request.Email),
                    MessageBuilder.BuildHtmlMessage(request.Lang, otp));
